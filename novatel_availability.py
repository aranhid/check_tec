...
stard_date = datetime(2022, 3, 11).date()
...
for key in date_list.keys():
    ...
    image_dir = os.path.join(image_path, key)
    if not os.path.exists(image_dir):
        os.mkdir(image_dir)
    image_file = os.path.join(image_dir, 'image.png')
    my_arg = f'{sys.executable} check_availability.py --files {" ".join(day_files)} --interval {interval} --window-size {window_size} --max-gap-num {max_gap_num} --plot-file {image_file} --nav-file {brdc_file} --cutoff {cutoff}'
