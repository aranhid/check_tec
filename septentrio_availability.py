...
for dir in dirs:
    ...
    image_file = os.path.join(new_path, 'image.png')
    my_arg = f'{sys.executable} check_availability.py --files {" ".join(files_to_check)} --interval {interval} --window-size {window_size} --max-gap-num {max_gap_num} --plot-file {image_file} --nav-file {nav_file} --cutoff {cutoff}'
